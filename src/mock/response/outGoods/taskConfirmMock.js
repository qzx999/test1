...
export const getList = options => {
    ...
    let taskConfirm = JSON.parse(localStorage.getItem('taskConfirm'));
    for(let i = 1; i<=15; i++) {
        responseList.push({
    ...
    if(taskConfirm == null || taskConfirm.length == 0) {
        localStorage.setItem('taskConfirm',JSON.stringify(responseList));
        response = {
            code: 200,
            msg:'查询成功',
            data:{
